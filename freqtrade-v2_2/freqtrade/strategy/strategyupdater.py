...
class StrategyUpdater:
    ...
    def start(self, config: Config, strategy_obj: dict) -> None:
        """
        Run strategy updater to update a strategy to v3 with AST module.
        :return: None
        """
        ...
        # read the file
        with Path(source_file).open("r", encoding="utf-8") as src_file:
            old_code = src_file.read()
        ...
        with Path(source_file).open("w") as src_file:
            src_file.write(new_code)
    ...
    def modify_ast(self, tree):  # noqa
        """
        Traverse the AST and update the names and functions as needed.
        """
...
class NameUpdater(ast_comments.NodeTransformer):
    def generic_visit(self, node):
        """
        Recursively visit all nodes in the AST, handling keyword, list, and AST types.
        """
        ...
        for field, old_value in ast_comments.iter_fields(node):
            if isinstance(old_value, list):
                old_value[:] = [
                    self.visit(value) if isinstance(value, ast_comments.AST)
                    else value for value in old_value
                ]
    ...
    def visit_Expr(self, node):
        """
        Handle expressions and replace outdated element names.
        """
    ...
    def check_dict(current_dict: dict, element: str):
        return current_dict.get(element, element)

    def visit_arguments(self, node):
        """
        Visit argument nodes and update names according to the mapping.
        """
    ...
    def visit_Name(self, node):
        """
        Visit name nodes and rename variables and identifiers according to the
        mapping.
        """
