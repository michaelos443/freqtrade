...
class FreqtradeBot(LoggingMixin):
    ...
    def _validate_trade_for_exit(self, trade: Trade) -> bool:
        """
        Check if the trade is valid for exit.
        Returns True if the trade is valid for exit, False otherwise.
        """
        if trade.has_open_orders and trade.has_open_sl_orders:
            if not self.wallets.check_exit_amount(trade):
                ...
                return True
            # Trade cannot be exited if there are open orders or SL orders
            return False
        return True
    

    def _handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        """
        Handles stoploss on exchange logic.

        :param trade: The trade to handle stoploss for.
        :return: True if stoploss was handled successfully, else False.
        """
        try:
    ...
    def exit_positions(self, trades: List[Trade]) -> int:
        ...
        :param trades: List of trades to exit
        :return: Number of trades closed.
        """
        trades_closed = 0
        for trade in trades:
            if not trade.is_open:
                logger.warning(f"Trade {trade} is not open, not exiting.")
                continue
            if not self._validate_trade_for_exit(trade):
    ...
    def handle_trade(self, trade: Trade) -> bool:
        ...
        :param trade: Trade object to handle.
        :return: True if trade has been sold/exited_short, False otherwise
        :raise: DependencyException if trade is not open.
        """
        if not trade.is_open:
        ...
        enter, exit_, exit_tag = self._get_exit_signal(trade)
    ...
    def _check_and_execute_exit(
        ...
        """
        Evaluates if an exit signal is present and executes the trade exit if so.
        :param trade: Trade object to handle.
        :param exit_rate: Current exit rate
        :param enter: Enter signal
        :param exit_: Exit signal
        :param exit_tag: Exit tag
        :return: True if trade has been sold/exited_short, False otherwise.
        :raise: PricingError if exit_rate is invalid.
        """
        if not instance(trade, Trade):
            raise TypeError(f"trade is not a Trade: {trade}")
        if exit_rate <= 0 or math.isnan(exit_rate):
            raise PricingError(f"Invalid exit_rate: {exit_rate} for trade: {trade}")
        if not isinstance(enter, bool) or not isinstance(exit_, bool):
            raise TypeError(f"enter and exit_ must be booleans. Got {enter} and {exit_}")
        if exit_tag is not None and not isinstance(exit_tag, str):
            raise TypeError(f"exit_tag must be a string. Got {exit_tag}")
        # Log initial check.
        logger.debug(f"Checking exit conditions for trade: {trade} with exit_rate: {exit_rate}")
        # Fetch the conditions from the strategy.
        try:
            exit_conditions: list[ExitCheckTuple] = self.strategy.should_exit(
                trade,
                exit_rate,
            ...
            logger.exception(f"Error while fetching exit conditions: {e}")
            return False
        if not exit_conditions or not any([x.exit_flag for x in exit_conditions]):
            logger.debug(f"Found no exit conditions or exit flags for trade: {trade}")
            return False
        for should_exit in exit_conditions:
            if should_exit.exit_flag:
                ...
                try:
                    ...
                    if exited:
                        ...
                        logger.info(f"Exit for {trade.pair} was not executed.")
                except Exception as e:
                    logger.error(f"Error during trade exit execution for {trade.pair}: {e}")
                    return False
        return False

    def _get_exit_signal(self, trade: Trade) -> Tuple[bool, bool, Optional[str]]:
        """
        Retrieves the exit signal for the given trade.

        :param trade: The trade to get the exit signal for.
        :return: A tuple containing enter, exit, and exit_tag.
        """
        if self.config.get("use_exit_signal", True) or self.config.get(
            "ignore_roi_if_entry_signal", False
        ):
            ...
            return self.strategy.get_exit_signal(
                trade.pair, self.strategy.timeframe, analyzed_df, is_short=trade.is_short
            )
    ...
    def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        :param trade: Trade object to create stoploss order for.
        :param stop_price: Price to use for stoploss order.
        :return: True if the order succeeded, and False in case of problems.
        """
        if not trade.is_open:
            logger.warning(f"Trade {trade} is not open, not creating stoploss order.")
            return False
        if not isinstance(stop_price, (float, int)):
            logger.warning(f"Invalid stop_price type: {stop_price} for trade: {trade}")
            return False
        if stop_price <= 0:
            logger.warning(f"Invalid stop_price: {stop_price} for trade: {trade}")
            return False
        logger.info(f"Creating stoploss on exchange for {trade.pair} at {stop_price}")

        try:
            # Create stoploss order on exchange
            ...
            # Update the trade with the stoploss order
            order_obj = Order.parse_from_ccxt_object(
                stoploss_order, trade.pair, "stoploss", trade.amount, stop_price
            )
            trade.orders.append(order_obj)
            logger.info(f"Stoploss order created for {trade.pair} at {stop_price}")
            return True
            ...
            logger.exception(
                f"Unexpected error while placing a stoploss order on exchange: {exc}."
            )
        return False
    ...
    def handle_cancel_order(
        ...
        :raise: DependencyException if order is neither entry nor exit order
        """
        if not order or not order_obj:
            logger.warning(f"Order or order_obj is None for trade {trade}.")
            return
        if trade and not trade.is_open:
            logger.warning(f"Trade {trade} is closed, not cancelling order.")
            return
        if order["side"] == trade.entry_side:
            ...
            logger.error(f"Order {order['id']} for {trade.pair} is neither entry nor exit order.")
            raise DependencyException(
                f"Order {order['id']} for {trade.pair} is neither entry nor exit order."
            )
    ...
    def emergency_exit(
        ...
        """
        Force exit a trade at a given price.
        :param trade: Trade object.
        :param price: Price to sell at.
        :param sub_trade_amt: Optional sub-trade amount to sell.
        :raise: DependencyException if the emergency exit fails, the trade argument is None
        or the price argument is invalid.
        """
        if not trade:
            raise DependencyException("Trade is None.")
        if price <= 0:
            raise DependencyException(f"Invalid price: {price} for emergency exit of {trade}.")
        try:
            self.execute_trade_exit(
                trade,
                price,
    ...
    def replace_order_failed(self, trade: Trade, msg: str) -> None:
        """
        Handle a failed order replacement.
    ...
    def is_order_cancelled(self, order: CcxtOrder) -> bool:
        """
        Check if order is cancelled
        :param order: Order dict grabbed with exchange.fetch_order()
        :return: True if order is cancelled, False otherwise
        """
        return order.get("status") in constants.NON_OPEN_EXCHANGE_STATES
    ...
    def handle_cancel_enter(
        ...
        """
        Check if current analyzed entry order timed out and cancel if necessary.
        ...
        :return: True if trade was fully cancelled, False otherwise
        """
        was_trade_fully_canceled = False
        order_id = order_obj.order_id
        side = trade.entry_side.capitalize()

        if not self.is_order_cancelled(order):
            # Retrieve the filled amount from the order, defaulting to 0 if
            # not available.
            filled_val: float = order.get("filled", 0.0) or 0.0
            # Calculate filled stake amount.
            filled_stake = filled_val * trade.open_rate
            # Get the minimum stake amount for this trade.
            minstake = self.exchange.get_min_pair_stake_amount(
                trade.pair, trade.open_rate, self.strategy.stoploss
            )
            # If the filled amount is greater than 0 and the filled stake is less than the
            # minimum stake amount, do not cancel the order.
            if filled_val > 0 and minstake and filled_stake < minstake:
                ...
                return False
            # Attempt to cancel the order and retrieve the updated order information.
            ...
            if replacing:
                for _ in range(3):
                    if self.is_order_cancelled(order=corder):
                        logger.info(f"Order {order_id} for {trade.pair} cancelled.")
                        break
                    logger.info(
                        f"Order {order_id} for {trade.pair} not cancelled yet. "
    ...
    def handle_cancel_exit(
        ...
        """
        Exit order cancellation.
        The method cancels the exit order and updates the trade
    ...
    def execute_trade_exit(
        ...
        *,
        ...
        :param trade: Trade instance to execute exit for.
        :param limit: limit rate for the sell order
        :param exit_check: CheckTuple with signal and reason
        :param exit_tag: Optional exit tag to use for this exit.
        :param ordertype: Optional order type to use for this exit
        :return: True if it succeeds, False otherwise.
        """
        if not isinstance(trade, Trade):
            raise ValueError(f"trade is not a Trade: {trade}")
        if limit <= 0:
            raise PricingError(f"Invalid limit value: {limit} for trade: {trade}")
        
        # Update funding fees for futures
        trade.set_funding_fees(
            self.exchange.get_funding_fees(
                pair=trade.pair,
                amount=trade.amount,
        ...
        # Determine exit reason and type
        exit_reason = exit_tag or exit_check.exit_reason
        exit_type = "stoploss" if exit_check.exit_type in (
            ExitType.STOP_LOSS,
        ...
        # First cancelling stoploss on exchange ...
        trade = self.cancel_stoploss_on_exchange(trade)
        # Check if trade is still open - it could have been closed by the stoploss cancellation.
        if not trade.is_open:
            logger.warning(f"Trade {trade} is closed, not executing exit.")
            return False
        ...
        time_in_force = self.strategy.order_time_in_force.get(exit_type, "GTC")

        # Confirm trade exit based on the exit type and conditions:
        if (
            ...
            and not strategy_safe_wrapper(self.strategy.confirm_trade_exit, default_retval=True)(
                ...
                sell_reason=exit_reason,  # sellreason -> For compatibility, will be removed in future.
                current_time=now,
            )
        ):
        ...
        # Check for a similar open order: cancel if the amount or side differs.
        if trade.has_open_orders:
        ...
        logger.info(
            f"Created {order_type} order for {trade.pair} amount: {amount} id: {order['id']}"
        )
        ...
        *,
        ...
        while retries > 0:
            ...
            try:
                ...
                if retries > 0:
                    logger.info(f"Retrying to fetch order {order_id} in 1 second.")
                    sleep(1)
                    continue
                else:
                    return False
