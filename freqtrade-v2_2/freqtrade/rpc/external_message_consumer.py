...
DEFAULT_WAIT_TIMEOUT = 30  # in seconds
DEFAULT_PING_TIMEOUT = 10  # in seconds
DEFAULT_MESSAGE_SIZE_LIMIT = 8 << 20  # in megabytes
...
class ExternalMessageConsumer:
    ...
    def __init__(self, config: dict[str, Any], dataprovider: DataProvider):
        ...
        if not self._emc_config:
            raise ValueError("external_message_consumer config not found")
        if not isinstance(self._emc_config, dict):
            raise TypeError("external_message_consumer config must be a dictionary")

        self.enabled = self._emc_config.get("enabled", False)
        self.producers: list[Producer] = self._emc_config.get("producers", [])
        if not isinstance(self.producers, list):
            raise TypeError("producers config must be a list")
        ...
        if self.enabled:
            ...
            for producer in self.producers:
                if not isinstance(producer, dict):
                    raise TypeError("Each producer entry must be a dictionary")
                if producer["name"] in [p["name"] for p in self.producers if p != producer]:
                    raise ValueError(f"Producer names must be unique. Duplicate: {producer['name']}")

        self.wait_timeout = self._emc_config.get("wait_timeout", DEFAULT_WAIT_TIMEOUT)  # in seconds
        self.ping_timeout = self._emc_config.get("ping_timeout", DEFAULT_PING_TIMEOUT)  # in seconds
    ...
    def start(self):
        ...
        This is so we can run the main loop in the background and not block
        the main thread.
        """
        if self._thread and self._loop:
            logger.info("ExternalMessageConsumer already running")
            return
        ...
        # Run the main coroutine safely within the thread's event loop
    ...
    async def _main(self) -> None:
        ...
        try:
            ...
            pass  # Ignore if the task is cancelled
        finally:
    ...
    async def _create_connection(self, producer: Producer, lock: asyncio.Lock):
        ...
        """
        if not isinstance(producer, dict):
            raise ValueError("Producer must be a dictionary")

        required_keys = ["name", "host", "port", "ws_token"]
        if not all(key in producer for key in required_keys):
            raise ValueError(
                "Producer must contain all required keys"
            )
    ...
    def handle_producer_message(self, producer: Producer, message: dict[str, Any]):
        ...
        # Validate the message, makes sure it matches the schema
        try:
        ...
        # Get the message handler for this message type
    ...
    def _consume_whitelist_message(
            self, producer_name: str, message: WSMessageSchema) -> None:
        try:
    ...
    def _consume_analyzed_df_message(self, producer_name: str, message: WSMessageSchema):
        """
        Handle analyzed dataframe message from a producer.
