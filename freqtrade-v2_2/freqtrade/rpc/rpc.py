"""
This module defines classes for managing RPC communications in Freqtrade.
It includes functionality for handling trade status, configuration, and performance statistics.
"""
...
class RPCHandler:
    ...
    Defines the interface for RPC communication modules.
    """
    ...
    def name(self) -> str:
        ...
        :return: Name of the RPC handler.
        """
...
class RPC:
    ...
    def _rpc_show_config(
        ...
        :param config: Configuration object
        :param botstate: Current state of the bot
        :param strategy_version: Version of the strategy
    ...
    def _rpc_trade_status(self, trade_ids: list[int] | None = None) -> list[dict[str, Any]]:
        ...
        :param trade_ids: Specific trade-ids to return status for. If None, return all trades.
        :return: List of trade dictionaries with status information.
        :raises RPCException: If no open trade is found.
        """
        if trade_ids is not None and not isinstance(trade_ids, list):
            raise RPCException("trade_ids must be a list of integers or None.")
        ...
        for trade in trades:
            ...
            trade_dict = self._calculate_trade_metrics(trade)
            results.append(trade_dict)
        return results

    def _calculate_trade_metrics(self, trade: Trade) -> Dict[str, Any]:
        """
        Calculate trade metrics for a single trade, including profit, stoploss,
        and fiat conversions.

        :param trade: Trade object to calculate metrics for.
        :return: Dictionary containing trade metrics.
        """
        ...
        # Update trade dictionary with calculated metrics
    ...
    def _rpc_status_table(
        ...
        """
        Generate a table of trade statuses.

        :param stake_currency: Stake currency used by the bot
        :param fiat_display_currency: Fiat currency used for display
    ...
    def _rpc_stats(self) -> dict[str, Any]:
        ...
        def trade_win_loss(trade) -> str:
            """
            Determine if a trade is a win, loss, or draw based on its profit.

            :param trade: Trade object to evaluate.
            :return: String indicating whether the trade is a win, loss, or draw.
            """
        ...
        try:
            ...
            logger.error(f"Error generating trade statistics: {e}")
    ...
    def _rpc_trade_statistics(
        ...
        """
        Returns cumulative profit statistics for trades in database.
        ...
        :return: Dictionary containing trade statistics.
        """
        # Set the start date to the beginning of time if not provided
        start_date = datetime.fromtimestamp(0) if start_date is None else start_date

        # Get all trades that are closed or open after the start date
        trade_filter = (
        ...
        if not trades:
            logger.warning("No trades found for statistics.")
            return {}

        # Initialize lists and counters for trade statistics
        profit_all_coin = []
        profit_all_ratio = []
        profit_closed_coin = []
        profit_closed_ratio = []
    ...
    def __balance_get_est_stake(
        ...
        """
        Get estimated stake amount for a given coin.

        :param coin: Coin to get the stake amount for
        :param stake_currency: Stake currency used by the bot
        :param amount: Amount of the coin
        :param balance: Wallet balance for the coin
        :return: Tuple of estimated stake amount and estimated stake amount for the bot
        """
        est_stake = 0.0  # Estimate stake amount for the coin
        est_bot_stake = 0.0
        ...
        if coin == self._freqtrade.exchange.get_proxy_coin():
            # For the proxy coin, use the balance (or free balance in futures mode)
            est_stake = balance.free if is_futures else balance.total
            ...
            try:
                ...
                if rate:
                    # Calculate the estimated stake and bot stake.
                ...
                # Fallback to returning 0 for both values.
                pass
        return est_stake, est_bot_stake

    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict[str, Any]:
        """
        Returns current account balance per cryptocurrency, including fiat conversions.
        :param stake_currency: Stake currency used by the bot
        :param fiat_display_currency: Fiat currency used for display
        :return: Dictionary containing account balance per cryptocurrency.
        """
        currencies: list[dict] = []
        total = 0.0
        total_bot = 0.0

        # Get open trades and create a dictionary of open assets
        open_trades: list[Trade] = Trade.get_open_trades()
        open_assets: dict[str, Trade] = {t.safe_base_currency: t for t in open_trades}
        # Update wallets to get the latest balances
        self._freqtrade.wallets.update(require_update=False)
        # Get starting capital and its fiat equivalent
        ...
        for coin, balance in self._freqtrade.wallets.get_all_balances().items():
            if not balance.total and not balance.free:
                continue  # Skip coins with no balance
            ...
            if not est_stake and not est_stake_bot:
                # Skip coins with no stake and no bot owned amount
                continue
            currencies.append(
                {
                    "currency": coin,
        ...
        # Add position information for futures trading
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        """
        Handler for reloading the bot's configuration.

        :return: A dictionary containing the status message.
        """
        if self._freqtrade.state == State.RELOAD_CONFIG:
            return {"status": "already reloading config"}
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> dict[str, str]:
        ...
        :param trade_id: id of the trade to reload
