...
def __run_download(job_id: str, config_loc: Config):
    """Run the data download process in background"""
    try:
        ApiBG.jobs[job_id]["is_running"] = True
        # Import download_data inside the function to avoid circular imports
        from freqtrade.data.history.history_utils import download_data

        # Run the download process without database context
        with FtNoDBContext():
            ...
            def ft_callback(task) -> None:
                """
                Callback function to update the progress of the download process.

                :param task: The task object representing the progress of the download process.
                """
            ...
            # Create a progress tracker object
        ...
        logger.exception("Unexpected error during data download: %s", e)
