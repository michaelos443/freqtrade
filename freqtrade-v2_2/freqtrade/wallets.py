...
class Wallets:
    ...
    def get_free(self, currency: str) -> float:
        """
        Retrieves the free (available) balance for a given currency.

        Args:
            currency (str): The currency for which to retrieve the free balance.

        Returns:
            float: The free balance if available, otherwise returns 0.
        """
    ...
    def get_used(self, currency: str) -> float:
        """
        Retrieves the used balance for the given currency.

        Args:
            currency (str): The currency for which to retrieve the used balance.

        Returns:
            float: The used balance if available, otherwise returns 0.
        """
        # Retrieve the balance object for the given currency
        balance = self._wallets.get(currency)
        return balance.used if balance and balance.used else 0

    def get_total(self, currency: str) -> float:
        """
        Retrieves the total balance for the given currency.

        Args:
            currency (str): The currency for which to retrieve the total balance.

        Returns:
            float: The total balance if available, otherwise returns 0.
        """
        balance = self._wallets.get(currency)
    ...
    def get_collateral(self) -> float:
        ...
        if self._config.get("margin_mode") == "cross":
            ...
            return self.get_free(self._stake_currency) + collateral_sum
    ...
    def _update_dry(self) -> None:
        ...
        """
        # Clear the _wallets to reset closed trade balances
        _wallets = {}
        _positions = {}
        ...
        profits_and_stakes = [
            (trade.realized_profit, trade.stake_amount) for trade in open_trades
        ]
        tot_profit += math.fsum(profit for profit, _ in profits_and_stakes)
        tot_in_trades = math.fsum(stake for _, stake in profits_and_stakes)
        used_stake = 0.0

        if self._config.get("trading_mode", "spot") != TradingMode.FUTURES:
            pair_currencies = {
                trade.pair: self._exchange.get_pair_base_currency(trade.pair)
                for trade in open_trades
            }
            for trade in open_trades:
                curr = pair_currencies[trade.pair]
                used_stake += sum(
        ...
        self._set_wallet_data(_wallets, _positions)

    def _set_wallet_data(self, wallets: dict, positions: dict) -> None:
    ...
    def _update_live(self) -> None:
        ...
        for position in positions:
            ...
            if not position.get("side") or not position.get("collateral", 0.0):
    ...
    def _check_exit_amount(self, trade: Trade) -> bool:
        ...
        return trade.amount <= wallet_amount
    ...
    def validate_stake_amount(
        ...
        if min_stake_amount is not None and stake_amount < min_stake_amount:
            ...
            adjusted_stake = stake_amount * 1.3
            if adjusted_stake < min_stake_amount:
                ...
                if not self._is_backtest:
                    logger.info(
                        ...
                        f"{adjusted_stake:.8f}) < {min_stake_amount}), ignoring trade."
                    )
                return 0
