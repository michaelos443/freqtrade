...
"""
This module manages Telegram communication for Freqtrade, handling message formatting,
command processing, and bot interactions with error handling as well.
"""
...
class Telegram(RPCHandler):
    ...
    def _init_keyboard(self) -> None:
        ...
        valid_commands = [k.replace("$", "") for k in valid_keys]
        pattern = re.compile("(" + ")|(".join(valid_keys) + ")")
        ...
        if cust_keyboard:
            ...
            invalid_keys = [
                b for b in chain.from_iterable(cust_keyboard) if not pattern.match(b)
            ]
            if len(invalid_keys):
                err_msg = (
    ...
    def _exchange_from_msg(self, msg: RPCOrderMsg) -> str:
        ...
        """
        if not isinstance(msg, dict) or "exchange" not in msg:
            logger.error("Invalid message format for exchange extraction")
            raise ValueError("Invalid message format")

        if not isinstance(msg["exchange"], str):
            logger.error("Invalid exchange format in message")
            raise ValueError("Invalid exchange format")

        return f"{msg['exchange']}{' (dry)' if self._config['dry_run'] else ''}"

    def _add_analyzed_candle(self, pair: str) -> str:
        """
        Adds the analyzed candle to the message if configured.
        :param pair: The pair to get the analyzed candle for.
        :return: The formatted analyzed candle or an empty string.
        """
        candle_setting = (
            self._config["telegram"].get("notification_settings", {}).get("show_candle", "off")
        )
        candle_val = candle_setting.lower()
        if candle_val == "off":
            return ""
        ...
        if candle_val == "ohlc":
            try:
                ...
                if analyzed_df.empty:
                    return ""
                # Get the last candle from the analyzed dataframe
                ...
                logger.error(f"Error getting analyzed candle: {e}")
                return ""
        return ""

    def _validate_trade_message(self, msg: dict, required_keys: list) -> None:
        """
        Validates trade message structure with comprehensive checks.
        
        :param msg: Trade message dictionary
        :param required_keys: List of mandatory keys
        :raises ValueError: If any required key is missing
        """
        if not isinstance(msg, dict):
            raise ValueError("Trade message must be a dictionary")
        missing = [key for key in required_keys if key not in msg]
        if missing:
            raise ValueError(f"Missing keys in trade message: {', '.join(missing)}")

    def _format_entry_msg(self, msg: RPCEntryMsg) -> str | None:
        """
        Formats the entry message.
        :param msg: The message to format.
        :return: The formatted message or None if an error occurred.
        """
        try:
            # Validate the message structure
            self._validate_trade_msg(
                msg, ['trade_id', 'pair', 'direction', 'open_rate']
            )
            ...
            logger.error(f"Error formatting entry message: {e}")
            return None
    ...
    def _format_exit_msg(self, msg: RPCExitMsg) -> str:
        ...
        """
        # Calculate the duration between open and close times 
        duration = msg["close_date"].replace(microsecond=0) - msg["open_date"].replace(
            microsecond=0
        )
        # Get the duration time value in minutes.
        ...
        cp_extra = ""
        # Determine the exit wording based on the message type and sub-trade status
        ...
        # Add the current rate and exit rate to the message if applicable
