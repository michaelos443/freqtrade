...
class Edge:
    ...
    _metrics_cache: Dict[str, PairInfo] = {}
    _cached_pairs: dict[str, Any] = {}  # Keeps a list of pairs

    def __init__(self, config: Config, exchange, strategy) -> None:
        """Initializes Edge class with configuration, exchange, and strategy."""
        self.config = config
        ...
        self._stoploss_range = self._generate_stoploss_range()

        self._timerange: TimeRange = self._initialize_timerange()
        ...
        logger.info("Using stoploss range: %s ...", self._stoploss_range)

    def _validate_config(self) -> None:
        """Ensure configuration meets Edge requirements."""
        if self.config["max_open_trades"] != float("inf"):
            raise OperationalException("Edge requires max_open_trades to be unlimited (-1)")
        if self.config["stake_amount"] != UNLIMITED_STAKE_AMOUNT:
            raise OperationalException("Edge works only with unlimited stake amount")

    def _generate_stoploss_range(self) -> np.ndarray:
        """Generate stop-loss range from configuration parameters."""
        return np.arange(
    ...
    def _initialize_timerange(self) -> TimeRange:
        """Create timerange for historical data analysis."""
        return TimeRange.parse_timerange(
            f"{(dt_now() - timedelta(days=self._since_number_of_days)).strftime('%Y%m%d')}-"
        )

    @property
    def cached_pairs(self) -> dict[str, Any]:
    ...
    def _find_trades_for_stoploss_range(
            self, df: DataFrame, pair: str, stoploss_range: np.ndarray) -> list:
        ...
        result: List[Dict] = []
        # Iterate through stoploss range
